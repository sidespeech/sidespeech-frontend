...
const UserCollectionItemSmall = ({collection, onClick}: UserCollectionItemProps) => {
    return (
        <UserCollectionItemSmallStyled onClick={onClick}>
            <div className="avatar">
                <img src={collection?.media?.[0]?.thumbnail} alt={`${collection?.name} avatar`} />
            </div>
            <div className="title-wrapper">
                <h3 className="title">{collection?.name || "{No Name}"}</h3>
