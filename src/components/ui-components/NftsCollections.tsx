...
export default function NftsCollections({
    ...
    const handleCollectionShowing = (position: any) => {
        const updatedCollectionShowing = openCollection.map((item, index) => (index === position ? !item : item));
    ...
    const handleFilterCollection = (e: any) => {
        ...
        const filteredData = collections.filter((value) => value.name?.toLowerCase().includes(filter.toLowerCase()));
            ...
            return (
                <p className="mt-2 mb-3 text-left">
                    ...
                    <span className="selected">
                        {' '}
                        {selectedNfts ? _.sum(Object.values(groupedNfts).map((c) => c.length)) : 0}
                    </span>
    ...
    const renderNumberSelection = (collection: Collection) => {
        const isAllSelected = groupedNfts[collection.address]?.length === collection.nfts.length;
            ...
            <div className="right-side">
                <div className="selected">
                    {!profile && !showAvatarText && (
                        <>
                            Public : <span className="selected">{groupedNfts[collection.address]?.length | 0}/</span>
                        </>
                    )}
    ...
    return (
        <NftsCollectionsStyled>
            ...
            <div className={`nftCollectionContainer ${profile ? '' : 'padding-bottom'}`}>
                {loading && (
                    <div className="f-column align-center justify-center" style={{ margin: 'auto' }}>
                        <div className="mb-1">Fetching your NFTS...</div>
                        <FadeLoader loading={loading} color="var(--text)" />
                    </div>
                )}
                {filteredCollections &&
                            ...
                            <div className={`nftCollection`} key={index}>
                                    ...
                                    <div className="title-wrapper">
                                        <div className="minus-plus-icon">
                                            {openCollection[index] ? (
                                                ...
                                                >
                                                    <path
                                                        d="M5.96372 13.1627V7.72832H0.529297V5.91685H5.96372V0.482422H7.7752V5.91685H13.2096V7.72832H7.7752V13.1627H5.96372Z"
                                                        fill="#B4C1D2"
                                                        fillOpacity="0.4"
                                                    />
                                                </svg>
                                                ...
                                                >
                                                    <path
                                                        d="M0.824219 1.70746V0.177734H11.5323V1.70746H0.824219Z"
                                                        fill="#B4C1D2"
                                                        fillOpacity="0.4"
                                                    />
                                                </svg>
                                ...
                                <div className={`nfts ${openCollection[index] ? 'closed' : 'open'}`}>
                                    {collection.nfts.map((nft: NFT, index: number) => {
                                        ...
                                        const isSelected =
                                            !profile && groupedNfts[collection.address]?.some((c) => nftEqual(c, nft));
                                        const isAvatar = selectedAvatar && nftEqual(nft, selectedAvatar);
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="inner">
                                                    {(isSelected || isAvatar) && (
                                                        <div className="status">{!isAvatar ? 'Public' : 'Avatar'}</div>
                                                    )}
                                                    <img
                                                        src={
...
function nftEqual(nft: NFT, nft2: NFT) {
    return nft.token_address === nft2.token_address && nft.token_id === nft2.token_id;
}
