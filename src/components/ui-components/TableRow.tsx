...
export default function TableRow({ side, user }: { side: Side; user: Profile }) {
    ...
    return (
                ...
                <div className="flex">
                    ...
                    <span className="fw-400 size-12 ml-2 text-primary-light">
                        {user['username'].length > 25
                            ? user['username'].replace(user['username'].substring(4, 34), '...')
                            : user['username']}
                    </span>
                </div>
            </td>
        ...
        // <Button width={70} height={35} onClick={undefined} radius={10} background={'var(--disable)'} color={'var(--text)'}>Eject</Button>
        //   </div>
        // </div>
    );
}
