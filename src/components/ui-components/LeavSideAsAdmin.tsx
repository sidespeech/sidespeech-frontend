...
const LeavSideAsAdmin = ({
  className,
  id,
  style,
  handleLeaveSide,
  side,
}: LeaveSideAsAdminProps) => {
  ...
  const handleNewSubAdmin = (event: any) => {
    setNewSubAdmin(event.target.value);
  };
  ...
  return (
    <LeaveSideAsAdminStyled className={className} id={id} style={style}>
      ...
      <p>
        To leave a Side, you must define a sub-admin-user who will become the
        new administrator.
      </p>
