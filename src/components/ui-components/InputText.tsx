...
const InputContainer = styled.div<any>`
	...
	& .suggestions {
		position: absolute;
		top: ${props => (props.dropdownDirection === 'down' ? '35px' : 'auto')};
		bottom: ${props => (props.dropdownDirection === 'up' ? '35px' : 'auto')};
		left: 0px;
		z-index: 999;
		cursor: pointer;
		width: 100%;
		...
		background-color: ${props => (props.bgColor ? props.bgColor : 'var(--disable)')};
	}
...
const InputText = forwardRef((props: InputTextPropsType, ref: any) => {
	...
	const [suggestions, setSugesstions] = useState<any[]>(props.data || []);
	const [isHideSuggs, setIsHideSuggs] = useState(true);
	const [selectedVal, setSelectedVal] = useState('');

	const wrapperRef = useRef();
	const clickOutside = useClickOutside(wrapperRef);
	const { direction, determineDirection } = useDropdownDirection(wrapperRef, DROPDOWN_MAX_HEIGHT);

	useEffect(() => {
		if (clickOutside) setIsHideSuggs(true);
	}, [clickOutside]);
	...
	const handleChange = (e: any) => {
		...
		if (props.autocomplete) {
			...
			if (props.data) {
				setSugesstions(props.data.filter(i => i.toLowerCase().startsWith(input.toLowerCase())));
			}
		}
		props.onChange(e);
	};

	const handleClickAutocomplete = (ev: any) => {
		setSugesstions(props.data || []);
		determineDirection(ev);
		setIsHideSuggs(false);
	};
	...
	return (
		...
		>
			<Input
				...
				onFocus={
					props.type === 'date' ? () => setIsDate(true) : props.autocomplete ? handleClickAutocomplete : null
				}
				...
				radius={props.autocomplete && !isHideSuggs ? `${props.radius} ${props.radius} 0px 0px` : props.radius}
				ref={ref}
				size={props.size}
			...
			{props.type === 'date' && (
				<span className="absolute" style={{ ...props.iconRightPos, color: props.color, pointerEvents: 'none' }}>
					<svg
						style={{ transform: `scale(${props.iconSize || 1})` }}
						width="16"
						height="18"
