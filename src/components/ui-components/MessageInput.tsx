...
const MessageInput = forwardRef(
  (props: MessageInputPropsType, ref: React.Ref<Editor> | null) => {
    ...
    const handleUploadFile = async (image: File | undefined): Promise<void> => {
      try {
        ...
        if (imagesToUpload.length >= 4)
          throw new Error("You can upload up to 4 images at a time");
        ...
        if (Math.round(image.size / 1024) / 1000 > 2)
          throw new Error("Please insert an image up to 2Mb");
        ...
        if (uploadedUrl) {
          ...
          setImagesToUpload((previousState) => [
            ...previousState.filter(
              (img) => img.id !== imageBase64.substring(0, 256)
            ),
            {
              file: imageBase64,
              id: imageBase64.substring(0, 256),
    ...
    const handleRemoveImageFromArray = (imageId: string): void => {
      setImagesToUpload((prevState) =>
        prevState.filter((img) => img.id !== imageId)
      );
    };
    ...
    const handleSubmit = (): void => {
      ...
      if (imagesToUpload?.length) {
        const arrayOfImagesMarkdown = imagesToUpload
          .map((img) => img.url)
          .join("  ");
        ...
        >
          <EditorSyled
            ...
            wrapperClassName={`flex-1 message-input-wrapper ${
              imagesToUpload?.length ? "loading-images" : ""
            }`}
          />

          <div
            className="absolute flex align-center ml-3 mr-3"
            style={{ bottom: 10, right: 10, zIndex: 3 }}
          >
            <button
            ...
            {!!props.toolbar && (
              <button
                ...
                style={
                  toolbarhidden
                    ? {}
                    : {
                        borderBottom: "1px solid var(--text-secondary-dark)",
              ...
              >
                <i
                  ...
                  className="fa-sharp fa-solid fa-font"
                />
              </button>
            )}
        ...
        {!!imagesToUpload.length && (
          <div className="images-to-upload-wrapper">
            {imagesToUpload?.map((imageObject) => (
              <div
                className={`images-to-upload-item ${
                  imageObject.uploading ? "uploading" : ""
                }`}
              >
                <img src={imageObject.file} alt="" />
                {imageObject.uploading && (
                  <div className="message-input-spinner" />
                )}
                <div className="image-actions">
                  <button
                    onClick={() => handleRemoveImageFromArray(imageObject.id)}
                  >
