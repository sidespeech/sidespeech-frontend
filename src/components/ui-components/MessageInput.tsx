...
const MessageInputStyled = styled.div<{ disabled?: boolean }>`
	position: relative;
	min-height: 3rem;
	background-color: ${props => (props.disabled ? '#05162c' : '#020f20')};
	color: var(--text);
	scrollbar-width: none;
	border-radius: 10px;
	border: ${props => (props.disabled ? '' : '1px solid var(--disable)')};
	width: 100%;
...
const MessageInput = forwardRef((props: MessageInputPropsType, ref: React.Ref<Editor> | null) => {
	...
	return (
		<MessageInputStyled disabled={props.disabled}>
			<div className="input-editor-wrapper">
				<EditorSyled
					...
					placeholder={props.disabled ? 'You cannot send messages here' : props.placeholder}
