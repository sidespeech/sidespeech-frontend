...
const PollItem = ({ handleVote, isFirstItem, isThread, poll, sideId }: PollItemProps) => {
    ...
    const handleComment = async (value: string) => {
        ...
        console.error(error);
        toast.error('There has been an error when commenting this poll', {toastId: getRandomId()}) 
        }
    };
        ...
        <div className="poll-item_container">
            ...
            <div className={`flex justify-between align-start poll-description ${finishedVoting ? 'finished' : 'progress'}`}>
                <div className="f-column">
                    ...
                    <p className="progress-message size-12">In progress - End on {format(new Date(poll.endDate || '2022-09-11'), 'MM/dd/yyyy')}</p>
                    )}
                </div>
