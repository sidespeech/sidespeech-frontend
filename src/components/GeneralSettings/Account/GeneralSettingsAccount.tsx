...
export default function GeneralSettingsAccount() {
	...
	const handleNftChange = (selectedNft: NFT) => {
		...
		const index = checkedNfts.findIndex(
			nft => nft.token_address === selectedNft.token_address && nft.token_id === selectedNft.token_id
		);
		if (index !== -1) {
			...
			{userCollectionsData && user && (
				<div className="fade-in content-wrapper">
					...
					{formData.publicNfts && collections.length > 0 && (
						<div className={`fade-in-delay nft-collections-wrapper ${displayNftsCollection ? 'show' : ''}`}>
							...
							<div className="buttons-wrapper">
								<Button color={'var(--text)'} onClick={onSubmit} width={'100%'}>
									Save this selection
								</Button>
							</div>
						</div>
