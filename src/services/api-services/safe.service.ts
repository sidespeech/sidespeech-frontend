...
class SafeService extends BaseApiService {
	...
	async buySafeNft(safeSdk: Safe, signer: ethers.providers.JsonRpcSigner, nft_address: string) {
		...
		const contractData = await openseaService.getContractDataTestnet(
			'0xcf00947d6c04a28a4c40026a8e6b03f3d83a0022/114'
		);
	...
	async createRegularTransaction(
		signer: ethers.providers.JsonRpcSigner,
		to_address: string,
		value: string,
		provider: ethers.providers.Web3Provider
	...
	async getTransactionsByAddressByBlock(
		address: string,
		provider: ethers.providers.Web3Provider,
	...
	async getFormatedTransaction(hash: string, provider: ethers.providers.Web3Provider) {
		...
		const formattedTransaction: Transaction = {
			...
			gasFee: ethers.utils.formatEther(
				etherTransaction['gasPrice']!.mul(etherTransactionReceipt['gasUsed']).toString()
			),
