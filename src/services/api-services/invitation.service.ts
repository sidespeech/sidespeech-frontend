...
class InvitationService extends BaseApiService {
  ...
  async sendMultipleInvitations(invitations: Invitation[]): Promise<any> {
    const res = await this.post(`${BASE_URL}/invitation/many`).send(
      invitations
    );
    return res.body;
  }
  ...
  async getRequestsFromInvitations(
    userId: string,
    sideId: string
  ): Promise<any> {
  ...
  async acceptInvitation(invitation: any): Promise<any> {
    const res = await this.post(`${BASE_URL}/invitation/accepted`).send(
      invitation
    );
    return res.body;
  }
  ...
  async acceptRequest(invitation: any): Promise<any> {
    const res = await this.post(`${BASE_URL}/invitation/request/accepted`).send(
      invitation
    );
    return res.body;
  }
