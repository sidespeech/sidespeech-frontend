...
class AnnouncementService extends BaseApiService {
	...
	async createAnnouncement(announcement: any, creatorAddress: any, channelId: string): Promise<Announcement> {
	...
	async createAnnouncementAsAdmin(
		announcement: any,
		creatorAddress: any,
		channelId: string,
		sideId: string
	): Promise<Announcement> {
		const createAnnouncement = await this.post(`${BASE_URL}/announcement/admin`)
			.send({
				content: announcement,
				creatorAddress: creatorAddress,
				channelId,
