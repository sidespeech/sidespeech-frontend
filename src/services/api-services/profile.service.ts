...
class ProfileService extends BaseApiService {
	...
	async joinSide(userId: string, sideId: string, role: number): Promise<Profile> {
	...
	async updateProfile(id: string, profile: Partial<Profile>): Promise<Profile> {
		const res = await this.patch(`${BASE_URL}/profile/update/${id}`).send(profile);
		return new Profile(res['body']);
	}
	async updateProfilePicture(id: string, profilePicture: NFT): Promise<Profile> {
	...
	/**
	 *
	 * @description set or unset the profile from side blacklist
	 * @param id id of the side
	 * @param profileId profile to update
	 * @param value value for blacklisting
	 * @returns updated profile
	 */
	async blacklistProfile(id: string, profileId: string, value: boolean): Promise<any> {
		const res = await this.patch(`${BASE_URL}/profile/blacklist/${id}`).send({ profileId, value });
		return new Profile(res.body);
	}
}
